...
1. As Ravi, on the Service Operations Workspace, find the incident pre-seeded for this lab by its short description — **"ZScaler tunnel dropping — cannot reach internal apps"** (`INC0010015` on this instance, though the exact number may differ on yours).
...
3. In the **Assigned to** field, type `ai` and select the AI Specialist you named in Exercise 1 (e.g., **Athena Service Desk AI Specialist** — your first name, plus the fixed last name "Service Desk AI Specialist") from the dropdown.
